...
# Path to the internal CA bundle used to verify the valuation API certificate.
...
def fetch_vehicle_valuation(vin):
    ...
    resp = requests.get(
        ...
        verify=VALUATION_CA_BUNDLE if VALUATION_CA_BUNDLE else True,
        timeout=10,
    )
...
def extract_vehicle_sales_data(region_filter=None):
    ...
    for idx, row in df.iterrows():
        try:
            ...
            logger.warning("Valuation lookup failed for VIN %s", row["vin"], exc_info=True)
