...
# Path to the CA bundle used to verify the enrichment API certificate.
VALUATION_API_CA_BUNDLE = os.getenv("VALUATION_API_CA_BUNDLE")


def _get_valuation_api_key():
    """Return the enrichment API key from the environment."""
...
def fetch_vehicle_valuation(vin):
    ...
    resp = requests.get(
        ...
        headers={"X-Api-Key": _get_valuation_api_key()},
